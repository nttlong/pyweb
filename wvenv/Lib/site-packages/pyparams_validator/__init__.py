    ...
    def __init__(self, params):
        ...
        if isinstance (params, tuple):
            if isinstance(params[0],list):
                self.data_type = list
                if params[0].__len__()>0:
                    detail_info = __GOBBLE__.consume_data (params[0][0], None)
        ...
        for item in list (input_fields):
            ...
            if isinstance(data[item],list):
                for _item in data[item]:
                    try:
                        self.data[item].validate (_item, item)
                        self.data[item].validate_data_type (_item, item)
                        index += 1
                    except exceptions.MissingFields as ex:
                        ret_fields =[]
                        for x in ex.fields:
                            y = x.split ('.')
                            _x = y[y.__len__()-1]
                            _y = x[0:x.__len__ () - _x.__len__ ()]
                            _y += "[{0}]".format (ex.fields.index (x))
                            _y +="."+_x
                            ret_fields.append(_y)
                        ex.fields =ret_fields
                        ex.message = "'{0}' is require".format (",".join (ex.fields))
                        raise exceptions.MissingFields(ret_fields,index,None)
                    except exceptions.InvalidDataFields as ex:
                        ret_fields = []
                        for x in ex.fields:
                            y = x.split ('.')
