...
class __validator_class__(object):
    ...
    def __setattr__(self, key, value):
        ...
        if __data_type__ == "object" and not type(value) is pdmobject:
...
class pdmobject(__validator_class__):
        ...
        if data != {}:
            ...
            for k,v in data.items():
                if k[0:2] != "__" and k.count('.') == 0:
                    ...
                    if type(v) is dict:
                        setattr(self,k,pdmobject(v))
                    elif type(v) is list:
                        values = []
                        for x in v:
                            if type(x) is dict:
                                values.append(pdmobject(x))
                            else:
                                values.append(x)
    ...
    def __getattr__(self, item):
        ...
        return super(pdmobject, self).__getattr__(item)
    def __setattr__(self, key, value):
        super(pdmobject, self).__setattr__(key, value)
